...
export default function Page() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/inventory/categories?q=${encodeURIComponent(search)}&page=${page}&limit=20`)
      .then(r => r.json())
      .then(d => { if (d.success) { setItems(d.data.categories); setTotal(d.data.total); } })
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr><th className="px-4 py-3 text-left font-medium">Name</th><th className="px-4 py-3 text-left font-medium">Description</th><th className="px-4 py-3 text-left font-medium">Sort Order</th></tr>
              </thead>
