...
export default function Page() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/corrective-actions?q=${encodeURIComponent(search)}&page=${page}&limit=20`)
      .then(r => r.json())
      .then(d => { if (d.success) { setItems(d.data.actions); setTotal(d.data.total); } })
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">No corrective actions found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr><th className="px-4 py-3 text-left font-medium">Title</th><th className="px-4 py-3 text-left font-medium">Assigned To</th><th className="px-4 py-3 text-left font-medium">Due Date</th><th className="px-4 py-3 text-left font-medium">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-700">{item.title}</td>
                    <td className="px-4 py-3 text-gray-700">{item.assignee?.firstName + " " + item.assignee?.lastName || "—"}</td>
                    <td className="px-4 py-3 text-gray-700">{item.dueDate ? new Date(item.dueDate).toLocaleDateString() : "—"}</td>
