...
export default function MonitoringPage() {
  ...
  const ProgressBar = ({ label, value, color, icon: Icon }: { label: string; value: number; color: string; icon: any }) => (
    <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="ml-auto text-lg font-bold text-gray-900 dark:text-white">{value}%</span>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-gray-900 dark:text-white">Monitoring</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Real-time system performance metrics</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* DB & API */}
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-5">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">Database & API</h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Database className="w-5 h-5 text-gray-500" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Active Connections</span>
              </div>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{metrics?.dbConnections ?? '-'}</span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Clock className="w-5 h-5 text-gray-500" />
                <span className="text-sm text-gray-700 dark:text-gray-300">API P95 Latency</span>
              </div>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{metrics ? `${metrics.apiP95}ms` : '-'}</span>
            </div>
        ...
        {/* Cache & Queue */}
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-5">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">Cache & Queue</h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Server className="w-5 h-5 text-gray-500" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Cache Hit Rate</span>
              </div>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{metrics ? `${metrics.cacheHitRate}%` : '-'}</span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2.5">
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700 dark:text-gray-300">Queue Depth</span>
              </div>
              <span className="text-lg font-bold text-gray-900 dark:text-white">{metrics?.queueDepth ?? '-'}</span>
            </div>
      ...
      {/* Chart Placeholder */}
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-5">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">API Response Time Trend</h3>
        <div className="h-64 flex items-center justify-center bg-gray-50 dark:bg-slate-700/50 rounded-lg border border-dashed border-gray-300 dark:border-slate-600">
