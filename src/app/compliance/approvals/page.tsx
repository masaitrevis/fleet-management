...
export default function Page() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/approvals/requests?q=${encodeURIComponent(search)}&page=${page}&limit=20`)
      .then(r => r.json())
      .then(d => { if (d.success) { setItems(d.data.requests); setTotal(d.data.total); } })
    ...
    <div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-500">No approval requests found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr><th className="px-4 py-3 text-left font-medium">Workflow</th><th className="px-4 py-3 text-left font-medium">Type</th><th className="px-4 py-3 text-left font-medium">Entity</th><th className="px-4 py-3 text-left font-medium">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-700">{item.workflow?.name || "—"}</td>
                    <td className="px-4 py-3 text-gray-700">{item.entityType}</td>
                    <td className="px-4 py-3 text-gray-700">{item.entityId}</td>
