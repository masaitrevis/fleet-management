...
export default function Page() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/inventory/alerts?q=${encodeURIComponent(search)}&page=${page}&limit=20`)
      .then(r => r.json())
      .then(d => { if (d.success) { setItems(d.data.alerts); setTotal(d.data.total); } })
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr><th className="px-4 py-3 text-left font-medium">Type</th><th className="px-4 py-3 text-left font-medium">Title</th><th className="px-4 py-3 text-left font-medium">Read</th><th className="px-4 py-3 text-left font-medium">Resolved</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-700">{item.alertType}</td>
                    <td className="px-4 py-3 text-gray-700">{item.title}</td>
                    <td className="px-4 py-3 text-gray-700">{item.isRead ? "Yes" : "No"}</td>
                    <td className="px-4 py-3 text-gray-700">{item.isResolved ? "Yes" : "No"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
