...
export default function EditRoutePage() {
  ...
  useEffect(() => {
    fetch(`/api/routes/${routeId}`).then(r => r.json()).then(d => {
      if (d.success) {
        const r = d.data;
        setForm({
          name: r.name || '',
          ...
          startLat: r.startLat ? String(r.startLat) : '',
          startLng: r.startLng ? String(r.startLng) : '',
          endLat: r.endLat ? String(r.endLat) : '',
          endLng: r.endLng ? String(r.endLng) : '',
          distance: r.distance ? String(r.distance) : '',
          estimatedDuration: r.estimatedDuration ? String(r.estimatedDuration) : '',
          isActive: r.isActive,
        });
        setStops(r.routeStops || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/routes/${routeId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
      if (data.success) { router.push(`/dashboard/routes/${routeId}`); }
      else { alert(data.error?.message || 'Failed to update route'); }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/dashboard/routes/${routeId}`} className="p-2 rounded-lg hover:bg-gray-100">
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required value={form.startLocation} onChange={(e) => setForm({ ...form, startLocation: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">End Location *</label>
              <input required value={form.endLocation} onChange={(e) => setForm({ ...form, endLocation: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-2">
            {waypoints.map((wp, i) => (
              <div key={wp.id || i} className="flex items-center gap-3 border border-gray-200 rounded-lg p-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/dashboard/routes/${routeId}`} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
