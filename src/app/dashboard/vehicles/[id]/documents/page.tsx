...
import { ArrowLeft, FileText, Upload, X, Plus, Trash2, Calendar, AlertCircle } from 'lucide-react';
...
const docTypes = ['LOGBOOK', 'INSURANCE', 'ROAD_LICENSE', 'INSPECTION_CERTIFICATE', 'EMISSIONS_CERTIFICATE', 'IMPORT_DOCUMENTS', 'WARRANTY'];
...
export default function VehicleDocumentsPage() {
  ...
  const [formData, setFormData] = useState({ type: 'LOGBOOK', title: '', url: '', expiryDate: '', renewalDate: '', reminderDays: 30 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { setShowModal(false); setFormData({ type: 'LOGBOOK', title: '', url: '', expiryDate: '', renewalDate: '', reminderDays: 30 }); fetchData(); }
    } catch (error) { console.error(error); }
  };

  const handleDelete = async (docId: string) => {
    if (!confirm('Delete this document?')) return;
    try { const res = await fetch(`/api/vehicles/${id}/documents/${docId}`, { method: 'DELETE' }); if (res.ok) fetchData(); }
    catch (error) { console.error(error); }
  };
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2 text-sm">
                        ...
                        {doc.renewalDate && <span className="text-gray-500">Renewal: {new Date(doc.renewalDate).toLocaleDateString()}</span>}
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="url" required value={formData.url} onChange={(e) => setFormData({ ...formData, url: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="https://..." />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Renewal Date</label><input type="date" value={formData.renewalDate} onChange={(e) => setFormData({ ...formData, renewalDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" /></div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Reminder Days</label>
                <input type="number" value={formData.reminderDays} onChange={(e) => setFormData({ ...formData, reminderDays: Number(e.target.value) })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
              </div>
