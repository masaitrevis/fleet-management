...
import { ArrowLeft, Plus, Trash2, MapPin, Package, ClipboardCheck } from 'lucide-react';
...
export default function EditTripPage() {
    ...
    fetch(`/api/trips/${tripId}`).then(r => r.json()).then(d => {
      if (d.success) {
        ...
        setForm({
          ...
          estimatedEndTime: t.estimatedEndTime ? new Date(t.estimatedEndTime).toISOString().slice(0, 16) : '',
          estimatedDistance: t.estimatedDistance ? String(t.estimatedDistance) : '',
          notes: t.notes || '',
          priority: t.priority || 'NORMAL',
        });
        setStops(t.tripStops || []);
        setCargos(t.tripCargos || []);
        setChecklist(t.tripChecklists || []);
      }
      setFetching(false);
    });
  }, [tripId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/trips/${tripId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
      if (data.success) { router.push(`/dashboard/trips/${tripId}`); }
      else { alert(data.error?.message || 'Failed to update trip'); }
  ...
  const updateCargo = (i: number, field: string, value: any) => { const u = [...cargos]; u[i] = { ...u[i], [field]: value }; setCargos(u); };
  ...
  const updateChecklistItem = (i: number, field: string, value: any) => { const u = [...checklist]; u[i] = { ...u[i], [field]: value }; setChecklist(u); };
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/dashboard/trips/${tripId}`} className="p-2 rounded-lg hover:bg-gray-100">
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
            </div>
            <div className="md:col-span-2">
            ...
            <div>
              ...
              <select value={form.vehicleId} onChange={(e) => setForm({ ...form, vehicleId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
            ...
            <div>
              ...
              <select value={form.driverId} onChange={(e) => setForm({ ...form, driverId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {stops.map((stop, i) => (
              <div key={stop.id || i} className="border border-gray-200 rounded-lg p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Stop Name</label>
                  <input value={stop.name} onChange={(e) => updateStop(i, 'name', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-600 mb-1">Address</label>
                  <input value={stop.address} onChange={(e) => updateStop(i, 'address', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="datetime-local" value={stop.scheduledArrival ? new Date(stop.scheduledArrival).toISOString().slice(0, 16) : ''} onChange={(e) => updateStop(i, 'scheduledArrival', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {cargos.map((cargo, i) => (
              <div key={cargo.id || i} className="border border-gray-200 rounded-lg p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Cargo Type</label>
                  <input value={cargo.cargoType} onChange={(e) => updateCargo(i, 'cargoType', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
                </div>
                <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-2">
            {checklist.map((item, i) => (
              <div key={item.id || i} className="flex items-center gap-3 border border-gray-200 rounded-lg p-3">
                <input value={item.item} onChange={(e) => updateChecklistItem(i, 'item', e.target.value)} className="flex-1 px-3 py-2 border border-gray-300 rounded-lg" />
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={`/dashboard/trips/${tripId}`} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</Link>
        </div>
      </form>
    </div>
  );
